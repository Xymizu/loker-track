export default function WishlistGrid({ jobs, onOpen, onMarkApplied }) {
  if (jobs.length === 0) {
    return (
        ...
        >
          ...
          <div className="flex items-center gap-2 mt-4 pt-3 border-t border-surface-border">
            <button
              ...
              className="flex-1 bg-brand-light text-brand text-xs font-semibold rounded-lg px-3 py-2 hover:bg-brand hover:text-white transition-colors"
            >
              Tandai sudah apply
            </button>
            <button
              onClick={() => onOpen(job)}
              className="text-ink-faint hover:text-ink text-xs font-medium px-2 py-2"
            >
              Ubah
            </button>
          </div>
